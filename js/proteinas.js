    ...
    $('.eliminar').click(function(){
         ...
         alertify.confirm('Eliminar', '¿Estas seguro de eliminar el registro?', function(){
                 ...
                 $.ajax({ 
                     type: "POST",
                     url: "../php/getCrud.php",
                     data:{ id: dataString, nombreTabla: "proteina", action: "delete" },
                     success: function(e) { 
                         $("#"+e.trim()).remove();
                     }
                 });
     ...
     $('#btn-guardarProteina').click(function(){
         ...
         if(nombreI == null || porcionI == null || nombreI.length == 0 || porcionI.length == 0 || /^\s+$/.test(nombreI) || /^\s+$/.test(nombreI) ){
            alert("Campos vacios...")
             ...
             $.ajax({ 
                 type: "POST",
                 url: "../php/getCrud.php",
                 data:{ nombre: nombreI, porcion: porcionI, nombreTabla: "proteina", action: "insert" },
                 success: function(e) { 
                     $("#"+e.trim()).remove();
                     $('#insertModal').modal('hide');
                     alertify.notify(e.trim()+' se agrego al catálogo de frutas', 'success', 5, function(){  console.log('dismissed'); });
                     window.location.replace('proteinas.php');
     ...
     $('.editar').click(function(){
        ...
        var idI = $(this).parent().attr('data');
         ...
         $('#btn-editarProteina').click(function(){
            ...
            $.ajax({ 
                type: "POST",
                url: "../php/getCrud.php",
                data:{ id: idI, nombre: nombreI, porcion: porcionI, nombreTabla: "proteina", action: "edit" },
                success: function(e) { 
