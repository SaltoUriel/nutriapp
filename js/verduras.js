    ...
    $('.eliminar').click(function(){
         ...
         alertify.confirm('Eliminar', '¿Estas seguro de eliminar el registro?', function(){
                 ...
                 $.ajax({ 
                     type: "POST",
                     url: "../php/getCrud.php",
                     data:{ id: dataString, nombreTabla: "verdura", action: "delete" },
                     success: function(e) { 
                         $("#"+e.trim()).remove();
                     }
                 });
     ...
     $('#btn-guardarVerdura').click(function(){
         ...
         if(nombreI == null || porcionI == null || nombreI.length == 0 || porcionI.length == 0 || /^\s+$/.test(nombreI) || /^\s+$/.test(nombreI) ){
            alert("Campos vacios...")
             ...
             $.ajax({ 
                 type: "POST",
                 url: "../php/getCrud.php",
                 data:{ nombre: nombreI, porcion: porcionI, nombreTabla: "verdura", action: "insert" },
                 success: function(e) { 
                     $("#"+e.trim()).remove();
                     $('#insertModal').modal('hide');
                     alertify.notify(e.trim()+' se agrego al catálogo de verduras', 'success', 5, function(){  console.log('dismissed'); });
                     window.location.replace('verdura.php');
     ...
     $('.editar').click(function(){
        ...
        var idI = $(this).parent().attr('data');
         ...
         $('#btn-editarVerdura').click(function(){
            ...
            $.ajax({ 
                type: "POST",
                url: "../php/getCrud.php",
                data:{ id: idI, nombre: nombreI, porcion: porcionI, nombreTabla: "verdura", action: "edit" },
                success: function(e) { 
